import { Route } from '@/app/router/routes/publications/$publicationId'
import { RootState } from '@/app/store'
import { useLazyGetByIdQuery } from '@/entities/plan'
...
export const PublicationPage = () => {
	...
	const [get] = useLazyGetByIdQuery()
	useEffect(() => {
		const fetch = async () => {
			const response = await get(publicationId).unwrap()
			setData(response)
		}
		fetch()
	}, [publicationId])
	...
	return (
		<div
			className={cn(
				'flex flex-col gap-2 border-4 border-primary p-4 h-full',
				data.actualDueDate &&
		...
		>
			...
			<div className='flex gap-2'>
				...
				<div className='flex flex-col border py-1 gap-1'>
					...
					<div className='border-2' />
					<div className='text-center px-2 flex gap-2'>
			...
			<div className='grid grid-cols-6 grid-rows-2 gap-x-4 text-center mt-auto'>
				<div className='col-span-2 row-span-1 border p-4 font-bold'>
